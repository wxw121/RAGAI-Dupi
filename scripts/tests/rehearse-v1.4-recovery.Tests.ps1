$scriptPath = Join-Path $PSScriptRoot "..\rehearse-v1.4-recovery.ps1"

Describe "V1.4 recovery rehearsal guardrails" {
    It "requires at least two fixture documents" {
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateFixtureOnly -FixtureDocumentCount 1 2>$null
        $LASTEXITCODE | Should Not Be 0
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateFixtureOnly -FixtureDocumentCount 2
    ...
    It "rejects incomplete and corrupt manifests" {
        ...
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateManifestOnly -ManifestBase64 $incomplete 2>$null
        $LASTEXITCODE | Should Not Be 0
        ...
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateManifestOnly -ManifestBase64 $encoded 2>$null
        $LASTEXITCODE | Should Not Be 0
    }
    ...
    It "accepts complete source target and vector evidence" {
        ...
        $encoded = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($manifest))
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateManifestOnly -ManifestBase64 $encoded
        $LASTEXITCODE | Should Be 0
    }

    It "only cleans resources created by this rehearsal" {
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateCleanupOnly -ResourceName 'v14-recovery-123'
        $LASTEXITCODE | Should Be 0
        & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateCleanupOnly -ResourceName 'production-kb' 2>$null
        $LASTEXITCODE | Should Not Be 0
    }

    It "emits a redacted artifact schema" {
        $json = & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ValidateArtifactOnly -ApiKey 'do-not-leak'
        $LASTEXITCODE | Should Be 0

...
1. Log in by entering the username/password of the configuration administrator. Do not use the local open mode. This session only creates the temporary administrator `e2e_gate_<runId>` among the `e2e` tenants.
...
### Most Recent Browser Gate (2026-07-14)

- URL: `http://localhost:8080`
- Browser: Playwright Chromium/Desktop Chrome configuration
- Result: `1 passed (4.5s)`; the core test flow took `3.7s`
...
### Most Recent Main-Flow Run (2026-06-19)

| Steps | State | Notes |
|---|---|---|
| 1-4 | PASS | Health, list, knowledge-base creation, and details |
| 5 Upload | PASS | Passes after the `DocumentService` timestamp fix |
| 6 Ingest | PASS | Requires a new KB using `embedding-2` and a Milvus collection dimension of 1024 |
| 7 Retrieval | PASS | At least one hit |
| 8 Chat SSE | PASS | DeepSeek streaming and the `LlmClient` SSE parsing fix |

**Run command:** `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/e2e-main-flow.ps1`

## Frequently Asked Questions (Page/Ingest)
...
### Ingest Failure 400 (Zhipu Embeddings)

Phenomenon: `Client error '400 Bad Request' for url '.../embeddings'`

- Common causes: **Old knowledge base:** `embeddingModel` is still `text-embedding-3-small` (the Worker calls the API based on KB metadata, not just by reading `.env`)
...
- Configure `EMBEDDING_*` (Zhipu) and `CHAT_*` (DeepSeek) in `deploy/.env`.
- `EMBEDDING_BASE_URL` is the root path of the API. **Do not** include the `/embeddings` suffix.
- After modifying `.env`, run `cd deploy && docker compose up -d --force-recreate worker api`.
...
- **Fix:** [`DocumentService.upload()`](../../services/api/src/main/java/com/dupi/rag/service/DocumentService.java) explicitly sets `createdAt` / `updatedAt` in the builder and pre-calculates `objectKey`.

...
V1.4.2增加了GovernanceOpsService和GET /api/v1/ops/governance-summary作为V1.4.1治理状态的只读OPS_ADMIN摘要。它聚合上传配额保留、摄取作业、摄取发件箱事件、摄取故障通知、矢量清理任务以及现有的审计、摄取和矢量警报摘要，而不改变任何这些记录。

响应部分包括：at、uploadQuota、ingestJobs、ingestOutbox、failureNotifications、vectorCleanup和alerts。V1.4.2可以为过期的上传尝试、过期的处理租约、失败的发件箱行和耗尽的失败通知附加开放条件警报代码，同时保留现有的AuditAlertResponse形状。

烟雾层在应用运行时之外。脚本/ smoke-governance-summary。ps1调用端点，验证JSON形状，并写入编校的证据。它的纠缠测试目前通过了4个。Node 16上的本地Web验证必须通过npm脚本，所以services/ Web /scripts/node16-webcrypto。CJS加载垫片；Raw和vitest不是支持的本地路径。
...
PostgreSQL拥有存档/作业/项目状态和' READY ' / ' RESTORING '可见性边界。专用的私有MinIO桶拥有规范归档对象。大量密集和活跃的稀疏集合被导出为确定性页面NDJSON。Worker通过其现有的回填边界提供一个稀疏目标。

存档流：获取知识库维护边界，等待摄取/评估/稀疏迁移活动停止，捕获确定性记录和原始对象、页面向量，验证每个项，然后写入manifest。json的最后。恢复流程：在分配一个目标之前进行验证，确定地重新映射标识符，有效地恢复，验证计数/模式/模型/维度/有效负载校验和，然后将目标更改为“READY”。

V1.2 JSON导出仍然是一种元数据迁移格式。它不包含原始对象字节或完整的矢量证据，也不是灾难恢复存档。

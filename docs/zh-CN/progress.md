...
- 已完成的设计与实施状态保留在这份受版本控制的进展日志中。
- 为控制本地资源消耗，真实浏览器 E2E、Docker Compose 发布演练、真实 load/latency 实验和生产聊天日志持久反馈队列延后到 post-V2.4。

## V2.0 RAG 质量体系封板可发布包（2026-07-26）

- 将原 V2.5-V3.0 后续重点挑选并收敛到 V2.0 封板 MVP：反馈候选、确定性答案裁判、实验矩阵、数据/索引治理、线上质量 SLO、canary 门禁和发布报告。
- 统一版本口径：当前 API/Web/lockfile 发布元数据为 V2.0（`2.0.0`）；V1.6b-V2.4 是已完成的本地质量里程碑，不再单独封包。
- 中文路线图：`docs/zh-CN/rag-quality-roadmap.md`；英文路线图：`docs/en/rag-quality-roadmap.md`。
- 设计文档：`docs/zh-CN/v2.0-rag-quality-closure-design.md`；实施文档：`docs/zh-CN/v2.0-rag-quality-closure-implementation.md`。
- 实现保持在现有 metrics JSON 与 Web 看板内；生产反馈表/事件流等重型能力保留到 Post-V2.0。
...
## V1.5.0 发布完成（2026-07-20）

API 和 Web 发布元数据均保持为 `1.5.0`。
- 增加了V1.5.0 版本说明和' docs/v1.5-release-runbook。md '用于Flyway V18-V20、Profile V2的推出、修订绑定评估、激活、监控和回滚。
- 最终的本地产品闸门通过了发布diff: API 482/482与95% JaCoCo闸门，Worker 96/96, Web 82/82，和1794模块生产构建。
- 撰写编辑配置和V1.5.0 版本扫描通过。扫描记录图像摘要‘ sha256:74043096b1740cad517a78f0792d79ea25a6bed99d17c1d8c96a7af98b05121c ’，大小为640,310,825字节，没有Python发现，以及22个精确的上游未修复的操作系统异常，到期日期为2026-08-15。
...
V1.5.0 RAG 质量升级P2完成：现有的VECTOR / HYBRID / RERANK模式和CLASSIC默认值保持兼容，现在添加了可过滤的配置文件v2索引超集，组合加权融合，修订绑定的RAG质量门和Web准备/门比较。

## 最近进展

2026-07-19 （V1.5.0 RAG 质量升级P2）

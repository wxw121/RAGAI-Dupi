...
public class RoleService {
    ...
    public static final List<PermissionMetadataResponse> DEFAULT_PERMISSION_DETAILS = List.of(
            ...
            permission("OPS_ALERT_NOTIFY", "告警通知", "允许把当前审计与运维告警发送到已配置的 Webhook。",
                    List.of("手动触发告警 Webhook 通知"),
                    List.of("不能修改 Webhook 配置或查看账号、角色数据")),
            ...
            permission("KB_READ", "知识库读取", "允许查看知识库和检索内容。",
                    List.of("查看知识库列表", "查看文档列表", "发起检索读取", "查看 RAG 评估用例和历史", "导出知识库快照"),
                    List.of("不能创建、编辑或删除知识库与文档")),
            permission("KB_WRITE", "知识库写入", "允许创建和编辑知识库配置。",
                    List.of("创建知识库", "编辑知识库基础配置", "导入知识库快照", "管理 RAG 评估用例"),
                    List.of("不能删除知识库")),
            ...
            permission("MAINTENANCE", "系统维护", "允许执行维护型动作。",
                    List.of("重新索引知识库", "重试摄入任务", "重试向量清理任务", "运行 RAG 评估"),
                    List.of("不能管理账号、角色或密码"))
    );
